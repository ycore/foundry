...
export const cfRetrieve = {
  ...
  wrangler: async wranglerPath => {
    ...
    const [project, error] = await go(JSON.parse(await fs.readFile(wranglerPath, 'utf-8')));
    // const packageJson = JSON.parse(fs.readFileSync(wranglerPath, 'utf-8'));
  ...
  kvNamespaces: async (prefix, environments, localProject) => {
    ...
    for (const environment of environments) {
      const kvNamespacePromises = localProject[environment].kv_namespaces.map(async localItem => {
        ...
        if (!kv_namespace) kv_namespace = kvNamespaceList?.find(namespace => namespace.title === kv_name);
        if (kv_namespace?.id) {
  ...
  d1Databases: async (prefix, environments, localProject) => {
    ...
    for (const environment of environments) {
      const d1DatabasePromises = localProject[environment].d1_databases.map(async localItem => {
        ...
        if (!d1_database) d1_database = d1DatabaseList?.find(database => database.name === db_name);
        if (d1_database?.uuid) {
