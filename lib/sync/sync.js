...
export const cfRetrieve = {
    ...
    remote: async (projectName) => {
        const [projectInfo, error] = await go(cfAPI.project.get(projectName));
        if (error)
            throw error;
        const remoteProject = { preview: {}, production: {} };
    ...
    wrangler: async (wranglerPath) => {
        const [project, error] = await go(import(wranglerPath));
        if (error)
            throw error;
        if (!project.default.env.production)
            project.default.env.production = {};
        if (project.default.vars)
            project.default.env.production.vars = project.default.vars;
        if (project.default.kv_namespaces)
            project.default.env.production.kv_namespaces = project.default.kv_namespaces;
        if (project.default.d1_databases)
            project.default.env.production.d1_databases = project.default.d1_databases;
        return project.default;
    },
    /*** Writes project configuration back to the local wrangler file. */
    write: async (projectPath, localProject) => {
        try {
            if (localProject.env.production?.vars)
                localProject.vars = localProject.env.production.vars;
            if (localProject.env.production?.kv_namespaces)
                localProject.kv_namespaces = localProject.env.production.kv_namespaces;
            if (localProject.env.production?.d1_databases)
                localProject.d1_databases = localProject.env.production.d1_databases;
...
export const cfSyncToRemote = {
    ...
    envVars: async (environments, localProject, remoteProject) => {
     ...
     */
    kvNamespaces: async (prefix, environments, localProject) => {
        ...
        for (const environment of environments) {
            const kvNamespacePromises = localProject[environment].kv_namespaces.map(async (localItem) => {
                ...
                if (kvNamespacesError)
                    throw kvNamespacesError;
                let kv_namespace = kvNamespaceList?.find((namespace) => namespace.id === localItem.id);
                if (!kv_namespace)
                    kv_namespace = kvNamespaceList?.find((namespace) => namespace.title === kv_name);
                if (kv_namespace?.id) {
     ...
     */
    d1Databases: async (prefix, environments, localProject) => {
        ...
        for (const environment of environments) {
            const d1DatabasePromises = localProject[environment].d1_databases.map(async (localItem) => {
                ...
                if (d1DatabaseError)
                    throw d1DatabaseError;
                let d1_database = d1DatabaseList?.find((database) => database.uuid === localItem.id);
                if (!d1_database)
                    d1_database = d1DatabaseList?.find((database) => database.name === db_name);
                if (d1_database?.uuid) {
...
const truncateString = (str, num = 40) => (str && str.length > num ? str.slice(0, num) + '...' : str);
