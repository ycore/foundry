...
// ../componentry/dist/impetus/toast/index.js
...
var NEVER = INVALID;
// ../../node_modules/remix-toast/dist/chunk-Z43GXJ4G.js
var toastMessageSchema = exports_external.object({
  message: exports_external.string(),
  description: exports_external.string().optional(),
  duration: exports_external.number().int().positive().optional(),
  type: exports_external.custom()
}).passthrough();
var toastMessageWithoutTypeSchema = exports_external.object({
  message: exports_external.string(),
  description: exports_external.string().optional(),
  duration: exports_external.number().int().positive().optional()
}).passthrough();
var flashSessionValuesSchema = exports_external.object({
...
async function flashMessage(flash2, headers, customSession) {
  const sessionToUse = customSession ? customSession : sessionStorage2;
  const session = await sessionToUse.getSession();
  session.flash(FLASH_SESSION, flash2);
  const cookie = await sessionToUse.commitSession(session);
  const newHeaders = new Headers(headers);
  newHeaders.append("Set-Cookie", cookie);
  return newHeaders;
}
async function redirectWithFlash(url, flash2, init, customSession) {
  return redirect(url, {
    ...init,
    headers: await flashMessage(flash2, init == null ? undefined : init.headers, customSession)
  });
}
async function dataWithFlash(data2, flash2, init, customSession) {
  return data(data2, {
    ...init,
    headers: await flashMessage(flash2, init == null ? undefined : init.headers, customSession)
  });
...
var dataWithToastFactory = ({ type, session }) => {
  return (data2, messageOrToast, init, customSession) => {
    const finalInfo = typeof messageOrToast === "string" ? { message: messageOrToast } : messageOrToast;
    return dataWithFlash(data2, { toast: { ...finalInfo, type } }, init, customSession != null ? customSession : session);
  };
};
...
// ../../node_modules/remix-toast/dist/middleware/index.js
var toastContext = unstable_createContext(null);
var sessionToastContext = unstable_createContext(null);
...
class Observer {
  constructor() {
    ...
    this.create = (data2) => {
      ...
      const id = typeof (data2 == null ? undefined : data2.id) === "number" || ((_data_id = data2.id) == null ? undefined : _data_id.length) > 0 ? data2.id : toastsCounter++;
      const alreadyExists = this.toasts.find((toast) => {
        return toast.id === id;
      });
      const dismissible = data2.dismissible === undefined ? true : data2.dismissible;
    ...
    this.promise = (promise, data2) => {
      ...
      let id = undefined;
      if (data2.loading !== undefined) {
        id = this.create({
          ...data2,
          promise,
          type: "loading",
          message: data2.loading,
          description: typeof data2.description !== "function" ? data2.description : undefined
        });
      }
      ...
      const originalPromise = p.then(async (response) => {
        ...
        } else if (data2.success !== undefined) {
          shouldDismiss = false;
        ...
        data2.finally == null || data2.finally.call(data2);
      });
    ...
    this.custom = (jsx, data2) => {
      const id = (data2 == null ? undefined : data2.id) || toastsCounter++;
      this.create({
        jsx: jsx(id),
        id,
        ...data2
...
function returnFailure(errors2) {
  return { data: null, errors: errors2 };
}
...
function flattenErrors(errors2) {
  if (Array.isArray(errors2)) {
    return errors2.flatMap((error) => error.messages).join("; ");
  }
  return Object.values(errors2).flatMap((fieldErrors) => fieldErrors.flatMap((error) => error.messages)).join("; ");
}

// ../forge/dist/logger/index.js
...
function createTotpTemplate(data2) {
  const { code } = data2;
...
async function sendAuthTotpEmail(params) {
  const { env, email, code, from } = params;
  return await sendEmail({ apiKey: env.EMAIL_API_KEY, to: email, from, template: createTotpTemplate({ code }) });
}
...
var EmailSchema = pipe(string(), trim(), toLowerCase(), email(), maxLength(254));
// src/email/templates/minimal-template.ts
function createMinimalTemplate(data2) {
  const { subject, message } = data2;
  const html = `<p>${message}</p>`;
  const text = `${message}`.trim();
  return { subject, html, text };
}
// src/email/templates/mock-template.ts
function createMockTemplate(data2) {
  const { subject, message, recipientName = "there" } = data2;
  const html = `
    <!DOCTYPE html>
    <html>
      <head>
...
//# debugId=330B239EB9EF6B8E64756E2164756E21
