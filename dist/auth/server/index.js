...
const challengeKvTemplate = (email2) => `challenge:${email2}`;
const challengeUniqueKvTemplate = (challenge) => `challenge-unique:${challenge}`;
...
async function cleanupChallengeSession(email2, context) {
  try {
    const { kv } = resolveAuthBindings(context);
    const challengeKey = challengeKvTemplate(email2);
    await kv.delete(challengeKey);
    return ok(void 0);
  } catch (error) {
    return err("Failed to cleanup challenge session", { email: email2, error });
  }
}
...
async function destroyAuthSession(request, context) {
  try {
    const sessionStorage = createAuthSessionStorage(context);
    const session = await sessionStorage.getSession(request.headers.get("Cookie"));
    const email2 = session.get("email");
    if (email2) {
      await cleanupChallengeSession(email2, context);
    }
...
async function createVerificationCode(email2, purpose, context, metadata) {
  try {
    ...
    logger.info("verification_code_created", { email: email2, purpose });
    return ok(code);
  } catch (error) {
...
async function verifyCode(email2, code, purpose, context) {
  try {
    ...
    await kv.delete(kvKey);
    logger.info("verification_code_verified", { email: email2, purpose });
...
async function sendRecoveryVerification(email2, context, verificationUrl) {
  try {
    ...
    logger.info("recovery_verification_sent", { email: email2 });
    return ok(void 0);
  } catch (error) {
...
async function requestAccountRecovery(email2, context, verificationUrl) {
  ...
  if (isError(userResult)) {
    logger.info("recovery_request_email_not_found", { email: email2 });
    return ok(null);
  }
        ...
        if (user?.pending) {
          ...
          if (user?.status !== "active") {
            ...
            if (isError(statusUpdateResult)) {
              ...
              user.status = "active";
              logger.info("status_restored_on_signin", { userId: user?.id, email: email2 });
            }
          }
        }
  ...
  const handlers = {
    signup: async (formData2) => {
      try {
        ...
        logger.info("signup_mode_detected", { email: email2, isRecoveryMode });
        const sessionResult = await getChallengeFromSession(request, context);
        if (isError(sessionResult)) {
          return sessionResult;
        }
        ...
        if (isRecoveryMode) {
          ...
          logger.info("recovery_mode_using_existing_user", { userId: user.id, email: email2 });
        } else {
        ...
        if (isRecoveryMode) {
          ...
          logger.info("recovery_passkey_registered", { userId: user.id, email: email2, recoveryTimestamp });
        }
        const cleanupResult = await destroyChallengeSession(session, context);
        if (isError(cleanupResult)) {
    ...
    verify: async (formData2) => {
      ...
      switch (purpose) {
        case "signup": {
          ...
          return ok({
            sessionCookie: isError(sessionUpdateResult) ? void 0 : sessionUpdateResult,
        ...
        case "recovery": {
          logger.info("recovery_email_verified", { userId: user.id, email: email2 });
          return ok({
