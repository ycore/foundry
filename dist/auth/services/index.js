...
import { requireCSRFToken as requireCSRFToken2 } from "@ycore/foundry/secure/services";
import { redirect as redirect4 } from "react-router";
async function signupLoader({ context }) {
  const token = requireCSRFToken2(context);
  const challenge = generateChallenge();
  const cookieResult = await createChallengeSession(context, challenge);
  ...
  return respondOk2({ token, challenge }, { headers: { "Set-Cookie": cookieResult } });
}
...
import { requireCSRFToken as requireCSRFToken3 } from "@ycore/foundry/secure/services";
...
async function verifyLoader({ request, context }) {
  const token = requireCSRFToken3(context);
...
//# debugId=1203F56A2E5E383564756E2164756E21
